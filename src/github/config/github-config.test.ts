...
Deno.test('GitHubConfigService - resetToDefaults', () => {
  ...
  const config = service.getConfig();
  assertEquals(config.defaultMachineType, DEFAULT_GITHUB_CONFIG.defaultMachineType);
  assertEquals(config.defaultIdleTimeoutMinutes, DEFAULT_GITHUB_CONFIG.defaultIdleTimeoutMinutes);
});
...
Deno.test('createGitHubConfigService factory function', () => {
  ...
  const customService = createGitHubConfigService({ defaultMachineType: 'premiumLinux' });
