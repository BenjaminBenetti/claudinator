...
export class GitHubConfigService implements IGitHubConfigService {
  ...
  validateConfig(config: Partial<GitHubConfig>): void {
    ...
    if (config.defaultLocation !== undefined) {
      const validLocations = ['EuropeWest', 'SoutheastAsia', 'UsEast', 'UsWest'];
    ...
    if (config.defaultIdleTimeoutMinutes !== undefined) {
      if (config.defaultIdleTimeoutMinutes < 5 || config.defaultIdleTimeoutMinutes > 240) {
        throw new Error('Idle timeout must be between 5 and 240 minutes');
      }
    }

    if (config.defaultRetentionPeriodMinutes !== undefined) {
      if (config.defaultRetentionPeriodMinutes < 60 || config.defaultRetentionPeriodMinutes > 43200) {
    ...
    if (config.baseRetryDelayMs !== undefined) {
      if (config.baseRetryDelayMs < 100 || config.baseRetryDelayMs > 10000) {
        throw new Error('Base retry delay must be between 100ms and 10 seconds');
      }
    }
    ...
    if (config.baseRetryDelayMs !== undefined && config.maxRetryDelayMs !== undefined) {
      if (config.maxRetryDelayMs <= config.baseRetryDelayMs) {
        throw new Error('Max retry delay must be greater than base retry delay');
      }
    }
  }
}
 ...
 */
export function createGitHubConfigService(initialConfig?: Partial<GitHubConfig>): IGitHubConfigService {
...
export function createConfigFromEnvironment(): Partial<GitHubConfig> {
  ...
  const retentionPeriod = Deno.env.get('GITHUB_DEFAULT_RETENTION_PERIOD_MINUTES');
  if (retentionPeriod) {
