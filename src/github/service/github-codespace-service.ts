...
export class GitHubCodespaceServiceImpl implements GitHubCodespaceService {
  ...
  async listCodespaces(repository?: string): Promise<Codespace[]> {
    try {
      ...
      throw new GitHubCodespaceServiceError(
        `Failed to list codespaces: ${error instanceof Error ? error.message : 'Unknown error'}`,
        'listCodespaces',
  ...
  async createCodespace(repository: string, branch?: string): Promise<Codespace> {
    try {
      await this.ensureAuthenticated();
      this.validateRepositoryFormat(repository);
      ...
      throw new GitHubCodespaceServiceError(
        `Failed to create codespace: ${error instanceof Error ? error.message : 'Unknown error'}`,
        'createCodespace',
        error instanceof Error ? error : undefined
      );
    }
  ...
  async getCodespaceStatus(name: string): Promise<Codespace> {
    try {
      ...
      throw new GitHubCodespaceServiceError(
        `Failed to get codespace status: ${error instanceof Error ? error.message : 'Unknown error'}`,
        'getCodespaceStatus',
        error instanceof Error ? error : undefined
      );
    }
  ...
  async cleanupOldCodespaces(repository?: string, olderThanDays = 7): Promise<number> {
    try {
      ...
      const codesToDelete = this.filterOldCodespaces(allCodespaces, olderThanDays);
      ...
      throw new GitHubCodespaceServiceError(
        `Failed to cleanup old codespaces: ${error instanceof Error ? error.message : 'Unknown error'}`,
  ...
  private async waitForCodespaceReady(name: string, maxAttempts = 10): Promise<void> {
