...
type MinimalRepository =
  Endpoints["GET /user/codespaces"]["response"]["data"]["codespaces"][0][
    "repository"
  ];
type SimpleUser =
  Endpoints["GET /user/codespaces"]["response"]["data"]["codespaces"][0][
 ...
 */
export function createMinimalRepository(
  overrides: Partial<MinimalRepository> = {},
): MinimalRepository {
  ...
  return {
    ...
    git_commits_url:
      `https://api.github.com/repos/${fullName}/git/commits{/sha}`,
    ...
    milestones_url:
      `https://api.github.com/repos/${fullName}/milestones{/number}`,
    notifications_url:
