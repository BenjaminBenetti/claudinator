...
import { GitHubApiError, createGitHubApiError, withGitHubApiErrorHandling } from './github-api-error.ts';
...
Deno.test('GitHubApiError getUserFriendlyMessage - 403 Insufficient scope', () => {
  const error = new GitHubApiError('Insufficient scope', 403, 'insufficient_scope');
...
Deno.test('createGitHubApiError - Network error', () => {
  ...
  assertEquals(error.message, 'Network test: Network error: getaddrinfo ENOTFOUND github.com');
...
Deno.test('withGitHubApiErrorHandling - Success on first attempt', async () => {
  ...
  const result = await withGitHubApiErrorHandling(mockOperation, 'Test operation');
