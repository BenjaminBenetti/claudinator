  ...
  public calculateTileLayout(containerWidth: number, containerHeight: number, tileCount: number): TileLayout {
    ...
    if (tileCount === 2) {
      return { 
        columns: 2, 
        rows: 1, 
        tileWidth: Math.floor(containerWidth / 2), 
    ...
    if (tileCount <= 4) {
      return { 
        ...
        rows: 2, 
        tileWidth: Math.floor(containerWidth / 2), 
        tileHeight: Math.floor(containerHeight / 2) 
      };
    }
    ...
    return {
      columns,
      rows,
      tileWidth: Math.floor(containerWidth / columns),
      tileHeight: Math.floor(containerHeight / rows)
    };
  }
